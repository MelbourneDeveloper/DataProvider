...
public static class SyncHelpers
{
    ...
    )
    {
        ...
        conn.Open();
        var result = SQLite.SyncLogRepository.FetchChanges(conn, fromVersion, batchSize);
    ...
    )
    {
        ...
        conn.Open();
        var result = Postgres.PostgresSyncLogRepository.FetchChanges(conn, fromVersion, batchSize);
        return result switch
        {
            Outcome.Result<IReadOnlyList<SyncLogEntry>, SyncError>.Ok<
                IReadOnlyList<SyncLogEntry>,
    ...
    private static long GetMaxVersionSqlite(string connectionString, ILogger logger)
    {
        using var conn = new SqliteConnection(connectionString);
        conn.Open();
        var result = SQLite.SyncLogRepository.GetMaxVersion(conn);
        return result switch
        {
            Outcome.Result<long, SyncError>.Ok<long, SyncError> ok => ok.Value,
            _ => 0,
        };
    }
    ...
    private static long GetMaxVersionPostgres(string connectionString, ILogger logger)
    {
        using var conn = new NpgsqlConnection(connectionString);
        conn.Open();
        var result = Postgres.PostgresSyncLogRepository.GetMaxVersion(conn);
        return result switch
        {
            Outcome.Result<long, SyncError>.Ok<long, SyncError> ok => ok.Value,
            _ => 0,
